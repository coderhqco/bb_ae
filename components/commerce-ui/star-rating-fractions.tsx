...
// Generate unique IDs per component instance to avoid hydration issues
const generateStarIds = (count: number, prefix: string) =>
  Array.from({ length: count }, (_, i) => `${prefix}-star-${i}`);
...
const StarRating_Fractions = ({
  ...
  // Generate stable IDs per component instance using useId
  const uniquePrefix = React.useId();
  const starIds = React.useMemo(
    () => generateStarIds(maxStars, uniquePrefix),
    [maxStars, uniquePrefix]
  );
