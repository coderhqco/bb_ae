...
function ProductVariant_02({
  ...
  const [internalSelectedVariant, setInternalSelectedVariant] = useState(defaultInitialVariant);
  ...
  const selectedVariantId = isVariantControlled ? controlledVariant : internalSelectedVariant;
  ...
  const selectedVariant = variants.find((v) => v.value === selectedVariantId) || variants[0];

  const currentImage = selectedVariant?.imageUrl || defaultImage;
  const currentPrice = selectedVariant.price;
  const currentSalePrice = selectedVariant.salePrice;
  const isOnSale = currentSalePrice !== undefined && currentSalePrice < currentPrice;

  // Get stock status from the selected variant
  const isInStock = selectedVariant.isInStock !== undefined ? selectedVariant.isInStock : true; // Default to in stock if not specified
  ...
  const variantsWithStockIndicator = variants.map((variant) => {
    const isVariantInStock = variant.isInStock !== undefined ? variant.isInStock : true;
    return {
      ...variant,
  ...
  return (
    <div className="grid mx-auto max-w-screen-lg grid-cols-1 gap-6 md:grid-cols-3 md:gap-8">
      ...
      <div className="flex flex-col gap-4 md:gap-2">
        <div className="rounded-md outline outline-rose-500">
          <div className="rounded-md rounded-b-none bg-gradient-to-r from-rose-500 to-red-600 p-3">
            <p className="font-semibold text-white">Get Free Shipping and 1-day delivery!</p>
          </div>
        ...
        <div className="flex flex-col gap-4 rounded-md border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="grid grid-cols-3 gap-2 text-xs sm:text-sm">
            <div className="col-span-1 text-gray-600 dark:text-gray-400">Sold by:</div>
            ...
            <div className="col-span-1 text-gray-600 dark:text-gray-400">Shipping:</div>
            <div className="col-span-2">
              <Link
                href="/"
                className="text-rose-600 hover:text-rose-700 hover:underline dark:text-rose-400 dark:hover:text-rose-300">
            ...
            <div className="col-span-1 text-gray-600 dark:text-gray-400">Return Policy: </div>
            <div className="col-span-2">
              <Link
            ...
            <div className="col-span-1 text-gray-600 dark:text-gray-400">Payment:</div>
